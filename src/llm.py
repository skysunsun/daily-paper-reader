    ...
    def _provider_name(self, base_url: str | None = None) -> str:
        try:
            ...
            if model.startswith('deepseek-'):
                return 'deepseek'
            # 第三方 OpenAI 兼容 API（例如中转服务）也按 deepseek 链路处理
            if 'openai' in url or model.startswith('gpt-') or model.startswith('o1') or model.startswith('o3'):
                return 'openai'
        except Exception:
            pass
        return 'llm'
...
class ClientFactory:
    ...
    def from_env():
        ...
        支持官方 DeepSeek 以及第三方中转 API（兼容 OpenAI Chat Completions 格式）。
        当 LLM_BASE_URL 指向第三方服务时，provider 仍可填 deepseek，系统会使用该 base_url。
        """
        model_env = (os.getenv('LLM_MODEL') or '').strip()
        if not model_env:
            raise ValueError("缺少必要环境变量: LLM_MODEL（格式为 'provider/model'，例如 'deepseek/deepseek-v4-flash'）")
        ...
        if provider == 'deepseek':
            # 如果用户提供了自定义 base_url（第三方中转），优先使用；否则用官方地址
