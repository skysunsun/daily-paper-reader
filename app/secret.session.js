...
(function () {
  ...
  const PROVIDER_TYPES = [
    {
      ...
      note: '使用 DeepSeek 官方 API，需要 DeepSeek API Key。',
    },
    {
      value: 'third_party',
      label: '第三方 DeepSeek（中转 API）',
      baseUrl: '',
      models: [],
      requiresCustomBaseUrl: true,
      note: '使用第三方中转 API（兼容 OpenAI Chat Completions 格式），需要自行填写 Base URL 和模型名。',
    },
  ];
  ...
  function setupOverlay(hasSecretFile) {
    ...
    const renderInitStep2 = (password) => {
      ...
      // 判断初始提供商类型：如果 baseUrl 不是官方地址，则为第三方
      const isOfficialUrl = !initialBaseUrl || initialBaseUrl === 'https://api.deepseek.com';
      const initialProviderType = isOfficialUrl ? 'deepseek' : 'third_party';
        ...
        <div class="secret-setup-step2-grid" style="font-size:13px;">
          <div class="secret-setup-step2-col">
            ...
            <div id="secret-setup-deepseek-section" class="secret-setup-step2-block">
              ...
              <p class="secret-setup-step2-note">
                DeepSeek 用于 query enrich、LLM refine、总结与聊天。支持官方 API 或第三方中转服务。
              </p>
              <div style="margin-bottom:8px;">
                <label style="font-size:12px; font-weight:500; display:block; margin-bottom:4px;">提供商类型</label>
                <select id="secret-setup-provider-type" class="secret-setup-select"></select>
              </div>
              ...
              <div id="secret-setup-third-party-fields" style="display:none;">
                <div class="secret-setup-input-row" style="margin-bottom:6px;">
                  <input
                    id="secret-setup-deepseek-base-url"
                    type="text"
                    autocomplete="off"
                    placeholder="第三方 API Base URL，例如 https://your-api.example.com"
                    style="width:100%; box-sizing:border-box; padding:6px 8px; font-size:13px;"
                  />
                </div>
                <div class="secret-setup-input-row" style="margin-bottom:6px;">
                  <input
                    id="secret-setup-deepseek-custom-model"
                    type="text"
                    autocomplete="off"
              ...
              <div id="secret-setup-official-fields">
                <div style="font-weight:500; margin-bottom:4px; display:flex; align-items:center; gap:4px;">
                  ...
                  <span class="secret-model-tip">!
                    <span class="secret-model-tip-popup">
                      当前支持 DeepSeek 官方 API 和第三方中转 API。<br/>
      ...
      const providerTypeSelect = document.getElementById('secret-setup-provider-type');
      ...
      const deepseekCustomModelInput = document.getElementById('secret-setup-deepseek-custom-model');
      const thirdPartyFields = document.getElementById('secret-setup-third-party-fields');
      const officialFields = document.getElementById('secret-setup-official-fields');
      ...
      // 填充提供商类型下拉
      providerTypeSelect.innerHTML = PROVIDER_TYPES
      ...
      deepseekCustomModelInput.value = initialDeepSeekModel;
      providerTypeSelect.value = initialProviderType;
      ...
      const selectedProviderType = () => {
        return normalizeText(providerTypeSelect.value || 'deepseek');
      };
      const isThirdParty = () => {
        return selectedProviderType() === 'third_party';
      };
      const selectedDeepSeekModel = () => {
        if (isThirdParty()) {
          return normalizeText(deepseekCustomModelInput.value || '');
        }
      ...
      const syncProviderFields = () => {
        ...
        thirdPartyFields.style.display = thirdParty ? 'block' : 'none';
        officialFields.style.display = thirdParty ? 'none' : 'block';
        if (!thirdParty) {
      ...
      const collectProviderDraft = () => {
        ...
        if (!model) {
          throw new Error('请选择或输入用于工作流总结的大模型。');
        }
        if (isThirdParty() && !baseUrl) {
          throw new Error('第三方 API 需要填写 Base URL。');
        }
        const chatModels = isThirdParty()
          ? [model]
          : getDefaultDeepSeekChatModels();
        const reranker = buildRerankerDraft(apiKey, baseUrl || getDefaultDeepSeekBaseUrl());
        return {
          providerType: 'deepseek',
          summaryApiKey: apiKey,
          summaryBaseUrl: baseUrl || getDefaultDeepSeekBaseUrl(),
          summaryModel: model,
      ...
      const buildPingEntries = () => {
        ...
        const baseUrl = selectedDeepSeekBaseUrl();
        if (!apiKey || !model) {
          throw new Error('请先填写 DeepSeek API Key 并选择/输入模型。');
        }
        if (isThirdParty() && !baseUrl) {
          throw new Error('第三方 API 需要填写 Base URL。');
        }
      ...
      bindResetOnInput([deepseekInput, deepseekModelSelect, deepseekBaseUrlInput, deepseekCustomModelInput, providerTypeSelect], resetDeepSeekStatus);
      bindResetOnInput([rerankerApiKeyInput, rerankerBaseUrlInput], resetRerankerTestStatus);
      providerTypeSelect.addEventListener('change', syncProviderFields);
      providerTypeSelect.addEventListener('change', resetDeepSeekStatus);
