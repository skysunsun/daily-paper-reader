(function (root, factory) {
  ...
  const inferChatApiProfile = (baseUrl, model) => {
    ...
    // 第三方 OpenAI 兼容 API：只要 model 以 deepseek- 开头或 baseUrl 非空，都按 deepseek 链路处理
  ...
  const isDeepSeekV4Model = (model) => {
    const normalizedModel = normalizeText(model || '').toLowerCase();
    return normalizedModel === 'deepseek-v4-flash' || normalizedModel === 'deepseek-v4-pro' || normalizedModel.startsWith('deepseek-');
  };
  ...
  const resolveMaxOutputTokens = ({ baseUrl, model } = {}) => {
    ...
    // 第三方 API 的 deepseek 模型也给予较大输出 token
    if (profile === 'deepseek') {
      return DEEPSEEK_V4_MAX_OUTPUT_TOKENS;
    }
    return null;
